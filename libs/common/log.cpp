...
namespace CHServer {
	...
	void CHLog::PrepareFile(const char* path, const char* prefix, uint32_t& day, uv_fs_t& fileHandle) {
		...
		if (fileHandle.data == NULL) {
			...
			snprintf(fileName, sizeof(fileName), "%s%s.%4d%02d%02d.log", path, prefix, timeStruct->tm_year + 1900, timeStruct->tm_mon + 1, timeStruct->tm_mday);
